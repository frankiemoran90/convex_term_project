function [x,x_debias,objective,times,debias_start,mses]= ...
         FISTA(y,A,tau,max_iters, varargin)
% This alorithm is based on the following with minor changes:
...
while cont_outer
  ...
  x = soft(yk + AT(y-A(x_previous)),tau);

  % Adding in FISTA 
  t = (1 + sqrt(1 + 4 * (t_p^2))) / 2;
  t_p = t;

  yk = x + ((t_p - 1)/t) * (x - x_previous);
